        ...
        <Box sx={{ bgcolor: 'background.paper', border: 1, borderColor: 'divider', borderRadius: 1 }}>
          ...
          {tab === 1 && (
            <GridPanel label="Create Ledger" onCreate={() => { setLedgerForm({ name: '', code: '', groupId: groups[0]?.id || '', ledgerType: 'GENERAL', openingBalance: 0, openingType: 'DEBIT', bankName: '', bankAccountNo: '', bankIfsc: '', bankBranch: '' }); setLedgerOpen(true); }}>
              <Table size="small"><TableHead><TableRow><TableCell>Name</TableCell><TableCell>Type</TableCell><TableCell>Code</TableCell><TableCell>Group</TableCell><TableCell>Bank</TableCell><TableCell>Opening</TableCell><TableCell>Status</TableCell></TableRow></TableHead><TableBody>{ledgers.map((ledger) => <TableRow key={ledger.id}><TableCell>{ledger.name}</TableCell><TableCell>{ledger.ledgerType}</TableCell><TableCell>{ledger.code}</TableCell><TableCell>{ledger.group.name}</TableCell><TableCell>{ledger.ledgerType === 'BANK' ? ledger.bankName || '-' : '-'}</TableCell><TableCell>{ledger.openingType} {Number(ledger.openingBalance).toFixed(2)}</TableCell><TableCell>{ledger.isActive ? 'Active' : 'Inactive'}</TableCell></TableRow>)}</TableBody></Table>
            </GridPanel>
          )}
          ...
          {tab === 6 && (
            <Stack spacing={2.5} sx={{ p: 2.5 }}>
              ...
              {ledgerReport && (
                <Table size="small"><TableHead><TableRow><TableCell>Date</TableCell><TableCell>Voucher</TableCell><TableCell>Narration</TableCell><TableCell align="right">Debit</TableCell><TableCell align="right">Credit</TableCell><TableCell align="right">Balance</TableCell></TableRow></TableHead><TableBody>{ledgerReport.entries.map((entry, index) => <TableRow key={`${entry.voucherNo}-${index}`}><TableCell>{entry.date.slice(0, 10)}</TableCell><TableCell>{entry.voucherType} {entry.voucherNo}</TableCell><TableCell>{entry.narration}</TableCell><TableCell align="right">{entry.debit.toFixed(2)}</TableCell><TableCell align="right">{entry.credit.toFixed(2)}</TableCell><TableCell align="right">{entry.balance.toFixed(2)}</TableCell></TableRow>)}</TableBody></Table>
              )}
            </Stack>
          )}
        </Box>
...
function VoucherTable({ vouchers }) {
  return <Table size="small"><TableHead><TableRow><TableCell>Date</TableCell><TableCell>Voucher</TableCell><TableCell>Narration</TableCell><TableCell>Lines</TableCell></TableRow></TableHead><TableBody>{vouchers.map((voucher) => <TableRow key={voucher.id}><TableCell>{voucher.voucherDate.slice(0, 10)}</TableCell><TableCell>{voucher.voucherType} {voucher.voucherNo}</TableCell><TableCell>{voucher.narration}</TableCell><TableCell>{voucher.lines.map((line) => `${line.ledger.name} ${line.type} ${Number(line.amount).toFixed(2)}`).join(' | ')}</TableCell></TableRow>)}</TableBody></Table>;
}
