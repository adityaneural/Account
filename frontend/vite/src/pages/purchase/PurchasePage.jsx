  ...
  return (
    <Grid container spacing={2.75}>
      ...
      <Dialog open={invoiceOpen} onClose={() => setInvoiceOpen(false)} fullWidth maxWidth="md">
        <Box component="form" onSubmit={(event) => { event.preventDefault(); save(() => api('/purchase/invoices', { method: 'POST', body: JSON.stringify({ ...invoiceForm, lines: invoiceForm.lines.map((line) => ({ ...line, quantity: Number(line.quantity), rate: Number(line.rate), taxRate: Number(line.taxRate || 0) })) }) }), 'Purchase invoice posted', () => setInvoiceOpen(false)); }}>
          <DialogTitle>Create Purchase Invoice</DialogTitle>
          <DialogContent><Stack spacing={2} sx={{ mt: 1 }}>
            <Grid container spacing={2}><Grid size={{ xs: 12, md: 6 }}><TextField select fullWidth label="Vendor" value={invoiceForm.vendorId} onChange={(event) => setInvoiceForm({ ...invoiceForm, vendorId: event.target.value })}>{vendors.map((vendor) => <MenuItem key={vendor.id} value={vendor.id}>{vendor.name}</MenuItem>)}</TextField></Grid><Grid size={{ xs: 12, md: 6 }}><TextField select fullWidth label="Warehouse" value={invoiceForm.warehouseId} onChange={(event) => setInvoiceForm({ ...invoiceForm, warehouseId: event.target.value })}>{warehouses.map((warehouse) => <MenuItem key={warehouse.id} value={warehouse.id}>{warehouse.branch.name} - {warehouse.name}</MenuItem>)}</TextField></Grid><Grid size={{ xs: 12, md: 4 }}><TextField fullWidth label="Invoice No" value={invoiceForm.invoiceNo} onChange={(event) => setInvoiceForm({ ...invoiceForm, invoiceNo: event.target.value })} /></Grid><Grid size={{ xs: 12, md: 4 }}><TextField fullWidth type="date" label="Invoice Date" value={invoiceForm.invoiceDate} onChange={(event) => setInvoiceForm({ ...invoiceForm, invoiceDate: event.target.value })} InputLabelProps={{ shrink: true }} /></Grid><Grid size={{ xs: 12, md: 4 }}><TextField fullWidth label="Supplier Invoice No" value={invoiceForm.supplierInvoiceNo} onChange={(event) => setInvoiceForm({ ...invoiceForm, supplierInvoiceNo: event.target.value })} /></Grid></Grid>
            {invoiceForm.lines.map((line, index) => <Grid container spacing={2} key={index}><Grid size={{ xs: 12, md: 5 }}><TextField select fullWidth label="Item" value={line.itemId} onChange={(event) => updateLine(index, 'itemId', event.target.value)}>{items.map((item) => <MenuItem key={item.id} value={item.id}>{item.name}</MenuItem>)}</TextField></Grid><Grid size={{ xs: 12, md: 2 }}><TextField fullWidth type="number" label="Qty" value={line.quantity} onChange={(event) => updateLine(index, 'quantity', event.target.value)} /></Grid><Grid size={{ xs: 12, md: 2 }}><TextField fullWidth type="number" label="Rate" value={line.rate} onChange={(event) => updateLine(index, 'rate', event.target.value)} /></Grid><Grid size={{ xs: 12, md: 2 }}><TextField fullWidth type="number" label="Tax %" value={line.taxRate} onChange={(event) => updateLine(index, 'taxRate', event.target.value)} /></Grid></Grid>)}
            <Button onClick={() => setInvoiceForm((current) => ({ ...current, lines: [...current.lines, { itemId: items[0]?.id || '', quantity: 1, rate: 0, taxRate: 0 }] }))}>Add Line</Button>
          </Stack></DialogContent>
