  ...
  return (
    <Grid container spacing={2.75}>
      ...
      <Dialog open={invoiceOpen} onClose={() => setInvoiceOpen(false)} fullWidth maxWidth="md">
        <Box component="form" onSubmit={(event) => { event.preventDefault(); save(() => api('/sales/invoices', { method: 'POST', body: JSON.stringify({ ...invoiceForm, lines: invoiceForm.lines.map((line) => ({ ...line, quantity: Number(line.quantity), rate: Number(line.rate), taxRate: Number(line.taxRate || 0) })) }) }), 'Sales invoice posted', () => setInvoiceOpen(false)); }}>
